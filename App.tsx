...
const App: React.FC = () => {
  ...
  // Historique interne de navigation pour le bouton retour du navigateur
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (session) {
        ...
        if (freshUser) {
          // isActive n'est pas dans le payload JWT → undefined doit être traité comme actif.
          // On bloque uniquement si explicitement false (compte désactivé côté admin).
          userToRestore = freshUser.isActive !== false ? freshUser : null;
          if (freshUser.isActive === false) authBridge.clearSession();
        } else {
  ...
  // Interception du bouton retour du navigateur — empêche de quitter l'application
  useEffect(() => {
    // Pousse un état initial pour que le premier "retour" reste dans l'app
    window.history.pushState({ tab: 'dashboard' }, '', window.location.href);

    const handlePopState = () => {
      // Re-pousser immédiatement pour bloquer la sortie de l'app
      window.history.pushState({ tab: 'dashboard' }, '', window.location.href);

      // Naviguer vers l'onglet précédent dans l'historique interne
      ...
      if (history.length > 1) {
        ...
        localStorage.setItem(ACTIVE_TAB_KEY, prevTab);
      }
    };
  ...
  // Navigation interne — met à jour l'historique pour le bouton retour
  const navigateToTab = (tab: string, meta?: any) => {
    tabHistoryRef.current = [...tabHistoryRef.current, tab];
    setNavigationMetadata(meta ?? null);
    localStorage.setItem(ACTIVE_TAB_KEY, tab);
    setActiveTab(tab);
    // Pousse un état pour que le bouton retour ait quelque chose à dépiler
        ...
        <div className="min-h-screen bg-slate-50">
          ...
          {pointageReminder && activeTab !== 'employee-pointage' && (
            <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-[300] flex items-center gap-4 px-6 py-4 bg-slate-900 text-white rounded-[2rem] shadow-2xl border border-indigo-500/30 max-w-sm w-full mx-4">
              ...
              <button
                onClick={() => { navigateToTab('employee-pointage'); setPointageReminder(false); }}
