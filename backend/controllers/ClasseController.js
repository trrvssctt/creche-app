...
export class ClasseController {
  static async list(req, res) {
    try {
      ...
      // Enseignant/Maîtresse : uniquement les classes où ils enseignent (prof principal ou intervenant)
      if (isTeacher(req)) {
      ...
      // Formater pour envoyer le nombre d'élèves (filtré par année si fournie)
  ...
  static async getById(req, res) {
    try {
      // Enseignant : vérifier que la classe lui appartient avant toute requête lourde
      if (isTeacher(req)) {
        if (!req.user.employeeId) {
        ...
        if (!classeIds.includes(req.params.id)) {
