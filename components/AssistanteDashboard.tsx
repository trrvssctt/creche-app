...
import {
  ...
  PiggyBank, Users, BarChart2, CalendarDays,
  CheckCircle2,
...
const AssistanteDashboard: React.FC<Props> = ({ user, currency, onNavigate }) => {
  ...
  const load = async () => {
    try {
      ...
      console.error('AssistanteDashboard error:', e);
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-[11px] font-semibold text-slate-500 mt-1">
            <span className="text-violet-600 font-black">Espace Assistante</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-2 bg-white rounded-3xl border border-slate-100 shadow-sm p-6">
          <div className="flex items-center justify-between mb-5">
            <SectionTitle icon={BarChart2} label="CA comptable mensuel" color="text-violet-600" />
            <button onClick={() => onNavigate?.('facturation')} className="text-[9px] font-black text-violet-600 flex items-center gap-1 hover:underline">
            ...
            <div className="h-48">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartComptable} barSize={14} barGap={2}>
                  ...
                  <Bar dataKey="encaisse" name="encaisse" fill="#8b5cf6" radius={[6, 6, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          )}
