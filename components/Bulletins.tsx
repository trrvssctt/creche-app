...
import {
  ...
  ArrowRight, User as UserIcon, BarChart2, School,
} from 'lucide-react';
...
export default function Bulletins({ user }: Props) {
  ...
  // Détection profil enseignant
  const userRoles = Array.isArray(user.roles) ? user.roles : [user.role];
  const isTeacherProfile = userRoles.some(r => r === 'ENSEIGNANT' || r === 'MAITRESSE');
  ...
  useEffect(() => {
    const fetchEleves = async () => {
      try {
        ...
        if (isTeacherProfile) {
          requests.push(apiClient.get('/classes', { params: { anneeScolaire: ANNEE_COURANTE } }));
        }
        const [elevesData, classesData] = await Promise.all(requests);

        const filtered = Array.isArray(elevesData) ? elevesData.filter((e: any) =>
          ['ACTIF', 'INSCRIT', 'ADMIS'].includes(e.statut) || !e.statut
        ) : [];
        setEleves(filtered);

        if (isTeacherProfile && classesData) {
          setMesClasses(Array.isArray(classesData) ? classesData : []);
        }
      } catch {
        setEleves([]);
        setMesClasses([]);
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="px-8 py-6 space-y-6">
        ...
        {/* ════ KPIs globaux ═════════════════════════════════════════════════════ */}
        {!(isTeacherProfile && !loading && mesClasses.length === 0) && <div className="grid grid-cols-4 gap-3">
          {[
        ...
        {viewStep === 'CLASSES' && (
          <>
            {/* Bandeau contextuel enseignant */}
            {isTeacherProfile && !loading && mesClasses.length > 0 && (
              <div className="flex items-start gap-3 px-5 py-4 bg-indigo-50 border border-indigo-100 rounded-2xl">
                ...
                <div>
                  <p className="text-xs font-black text-indigo-800 uppercase tracking-widest mb-1">Vos classes assignées</p>
                  <div className="flex flex-wrap gap-2">
                    {mesClasses.map((c: any) => (
                      <span key={c.id} className="inline-flex items-center gap-1.5 px-3 py-1 bg-white border border-indigo-200 rounded-full text-[10px] font-black text-indigo-700">
                        <span className="w-1.5 h-1.5 rounded-full bg-indigo-400 inline-block" />
                        {c.nom}
                        {c.enseignantId === (user as any).employeeId && (
                          <span className="px-1.5 py-0.5 bg-indigo-100 text-indigo-600 rounded text-[8px] font-black uppercase">Principal</span>
                        )}
                      </span>
                    ))}
                  </div>
            ...
            ) : isTeacherProfile && mesClasses.length === 0 ? (
              /* ── État vide enseignant sans classe assignée ── */
              <div className="flex flex-col items-center justify-center py-20 bg-white rounded-3xl border border-dashed border-slate-200">
                {/* Illustration animée */}
                <div className="relative mb-8">
                  <div className="w-28 h-28 bg-indigo-50 rounded-full flex items-center justify-center animate-pulse">
                    <div className="w-20 h-20 bg-indigo-100 rounded-full flex items-center justify-center">
                      <School size={40} className="text-indigo-300" />
                    </div>
                  </div>
                  {/* Icônes orbitales */}
                  <div className="absolute -top-2 -right-2 w-9 h-9 bg-amber-100 rounded-full flex items-center justify-center animate-bounce" style={{ animationDelay: '0.2s' }}>
                    <BookOpen size={16} className="text-amber-500" />
                  </div>
                  <div className="absolute -bottom-1 -left-3 w-8 h-8 bg-rose-100 rounded-full flex items-center justify-center animate-bounce" style={{ animationDelay: '0.5s' }}>
                    <GraduationCap size={14} className="text-rose-400" />
                  </div>
                  <div className="absolute top-6 -left-5 w-7 h-7 bg-emerald-100 rounded-full flex items-center justify-center animate-bounce" style={{ animationDelay: '0.8s' }}>
                    <Users size={13} className="text-emerald-500" />
                  </div>
                </div>

                <h3 className="text-base font-black text-slate-700 mb-2">Aucune classe assignée</h3>
                <p className="text-sm text-slate-400 text-center max-w-sm leading-relaxed mb-1">
                  Vous n'êtes actuellement rattaché à aucune classe en tant que professeur principal ou intervenant.
                </p>
                <p className="text-xs text-slate-300 font-bold text-center max-w-xs">
                  Contactez la direction pour être affecté à une ou plusieurs classes afin d'accéder aux bulletins.
                </p>
              </div>
