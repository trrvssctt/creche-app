...
const TeacherPortal: React.FC<TeacherPortalProps> = ({
  ...
  const myClasses = useMemo(() => {
    ...
    return classes.filter(c =>
      String(c.enseignantId) === empId ||
      (c.enseignantsMatiere || []).some(em => String(em.enseignantId) === empId)
    );
  }, [classes, empId]);
  ...
  const tabs: [Section, string, any][] = [
    ...
    ['ma-classe',    'Mes Classes',  GraduationCap],
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[11px] font-semibold text-slate-500 mt-1">
            <span className="text-indigo-600 font-black">{myClasses.length}</span> classe{myClasses.length !== 1 ? 's' : ''} assignée{myClasses.length !== 1 ? 's' : ''} · <span className="text-indigo-600 font-black">{myElevesCount}</span> élèves · {annee}
          </p>
        </div>
        <div className="flex items-center gap-2">
          <div className={`flex items-center gap-2 px-4 py-2.5 rounded-2xl border text-[10px] font-black uppercase tracking-widest ${pointage?.attendance?.clockIn ? 'bg-emerald-50 border-emerald-200 text-emerald-600' : 'bg-amber-50 border-amber-200 text-amber-600'}`}>
        ...
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
            {[
              ...
              { label: 'Présences',       desc: 'Saisir absences & retards', Icon: ClipboardList, color: 'bg-violet-50 text-violet-700', action: () => setSection('presences') },
          ...
          {myClasses.length > 0 && (
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
              <div className="px-5 py-4 border-b border-slate-50 flex items-center justify-between">
                <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Mes Classes — Aperçu</p>
        ...
        <div className="space-y-4">
          {myClasses.length === 0 ? (
            <div className="py-20 flex flex-col items-center gap-3 text-slate-400 bg-white rounded-2xl border border-dashed">
              ...
              <p className="text-[10px] font-bold uppercase tracking-widest">Aucune classe assignée</p>
            </div>
          ) : myClasses.map(c => {
                ...
                <div className="px-5 py-4 border-b border-slate-50 flex items-center justify-between flex-wrap gap-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[9px] font-semibold text-indigo-500">{getRoleInClass(c)} · {elevesC.length} élève{elevesC.length !== 1 ? 's' : ''}</p>
                    </div>
                  </div>
                  <button onClick={() => { setSelectedClassId(c.id); setSection('presences'); }}
        ...
        <div className="space-y-4">
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest block mb-1.5">Classe</label>
                <select value={selectedClassId} onChange={e => setSelectedClassId(e.target.value)}
                  ...
                  {myClasses.map(c => <option key={c.id} value={c.id}>{c.nom} · {getRoleInClass(c)}</option>)}
                </select>
              </div>
              <div>
              ...
              <div className="px-5 py-4 border-b border-slate-50 flex items-center justify-between flex-wrap gap-3">
                <div>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                    Présences — {myClasses.find(c => c.id === selectedClassId)?.nom}
                  </p>
