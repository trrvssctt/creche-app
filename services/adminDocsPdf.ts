...
function buildAutorisationSortieHtml(eleve: Partial<Eleve>, logoBase64: string): string {
  const nomComplet = `${eleve.prenom || ''} ${eleve.nom || ''}`.trim() || '—';
  const parent = eleve.parent1;
  const niveauLib = NIVEAUX_MAP[eleve.niveau || ''] || eleve.niveau || '—';
  ...
  return `<!DOCTYPE html><html lang="fr"><head><meta charset="UTF-8"><style>${SHARED_CSS}</style></head><body>
    ...
    <div class="text-block">
      ...
      <p style="margin-top:14px">autorise mon enfant à participer à toutes les sorties scolaires et activités extrascolaires
         organisées par l'établissement <strong>Le Toit des Anges</strong> durant l'année scolaire
         <strong>${getAnnee()}</strong>.</p>
      <p style="margin-top:14px">Je reconnais avoir été informé(e) des conditions d'encadrement et des mesures de sécurité
         mises en place par l'établissement.</p>
    </div>
...
export type DocAdminType = 'fiche_inscription' | 'certificat_scolarite' | 'fiche_sanitaire' | 'autorisation_sortie';

function buildHtml(type: DocAdminType, eleve: Partial<Eleve>, logoBase64: string): string {
  switch (type) {
